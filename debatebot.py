...
async def floor(ctx, *, side):
    ...
    with db.Session() as session:
        ...
        side2 = session.query(Storage).filter_by(side2_role=side).one_or_none()
        if side1 is not None:
            role1 = await discord.utils.get(ctx.guild.roles, name="{}".format(side1.side1_name))
            role2 = await discord.utils.get(ctx.guild.roles, name="{}".format(side2.side2_name))
            ...
            overrides.update(send_messages=True)
            otheroverrides = ctx.channel.overwrites_for(side)
            otheroverrides.update(send_messages=False)
            await ctx.send("Side {} has the floor".format(side))
        else:
